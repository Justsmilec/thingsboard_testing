...
public class TestingController  extends BaseController {
    ...
    @ResponseBody
    public Testing getTestingById(@PathVariable(TESTING_ID) String strTestingId) throws ThingsboardException {
        checkParameter(TESTING_ID, strTestingId);
        try {
            TestingId testingId = new TestingId(toUUID(strTestingId));
            return checkTestingId(testingId, Operation.READ);
        } catch (Exception e) {
    ...
    @RequestMapping(value = "/testing1", method = RequestMethod.GET)
    public Testing testing1(){
        System.out.println("e po po qenka kshu kjo");
        return null;
    }
    ...
    @ResponseBody
    public TestingInfo getTestingInfoById(@PathVariable(TESTING_ID) String strTestingId) throws ThingsboardException {
        checkParameter(TESTING_ID, strTestingId);
        try {
            TestingId testingId = new TestingId(toUUID(strTestingId));
            return checkTestingInfoId(testingId, Operation.READ);
        } catch (Exception e) {
            throw handleException(e);
        }
    }
    ...
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/testings", params = {"testingIds"}, method = RequestMethod.GET)
    @ResponseBody
    public List<Testing> getTestingsByIds(
            @RequestParam("testingIds") String[] strDeviceIds) throws ThingsboardException {
        checkArrayParameter("testingIds", strDeviceIds);
        try {
            SecurityUser user = getCurrentUser();
            TenantId tenantId = user.getTenantId();
            CustomerId customerId = user.getCustomerId();
            List<TestingId> deviceIds = new ArrayList<>();
            for (String strDeviceId : strDeviceIds) {
                deviceIds.add(new TestingId(toUUID(strDeviceId)));
            }
            ListenableFuture<List<Testing>> devices;
            if (customerId == null || customerId.isNullUid()) {
                devices = testingService.findTestingsByTenantIdAndIdsAsync(tenantId, deviceIds);
            } else {
                devices = testingService.findTestingsByTenantIdCustomerIdAndIdsAsync(tenantId, customerId, deviceIds);
            }
...
//
//    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
//    @RequestMapping(value = "/testing/info/{deviceId}", method = RequestMethod.GET)
//    @ResponseBody
